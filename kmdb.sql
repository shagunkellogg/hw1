...
SELECT movies.title, actors.actor_name, character.name 
FROM character
    INNER JOIN movies ON character.movie_id = movies.id
    INNER JOIN actors ON character.actor_id = actors.id
ORDER BY movies.year_released, movies.title;